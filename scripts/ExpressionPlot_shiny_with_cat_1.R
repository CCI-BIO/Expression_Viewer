  ...
  tryCatch(
    expr = {
      ...
      if(input$Select_plotfile_dot == "online_plot"){
        ...
        shinyjs::disable("ExpressionDownload_dot")
        shinyjs::disable("ExpressionDownload_dot_cat")
        print("offline")
...
observeEvent(
  c(input$selectFolder, input$Select_plotfile_dot),
  {
    output$plotarea_dot <- renderPlotly({NULL})
    shinyjs::disable("ExpressionDownload_dot")
    shinyjs::show("selectFolder")
    shinyjs::show("selectPatient_dot")
    shinyjs::show("selectgenes2_dot")
    if(input$Select_plotfile_dot == "online_plot" & input$selectFolder == "ExpressionAnalysis"){
      output$plotarea_dot <- renderPlotly({NULL})
      output$plotarea_dot_cat <- renderPlotly({NULL})
      output$message_2 <- renderText({NULL})
      shinyjs::disable("ExpressionDownload_dot")
      ...
      #dirLoc<-"R:/KCA/Projects/ZeroApp_multiple/Updated_files/ExpressionAnalysis/"
    }else if (input$Select_plotfile_dot == "online_plot" & input$selectFolder == "totalRNA") {
      output$plotarea_dot <- renderPlotly({NULL})
      output$plotarea_dot_cat <- renderPlotly({NULL})
      output$message_2 <- renderText({NULL})
      shinyjs::disable("ExpressionDownload_dot")
    ...
    }else if (input$Select_plotfile_dot == "online_plot" & input$selectFolder == "AGRF_trial") {
      output$plotarea_dot <- renderPlotly({NULL})
      output$plotarea_dot_cat <- renderPlotly({NULL})
      output$message_2 <- renderText({NULL})
      shinyjs::disable("ExpressionDownload_dot")
      ...
      #dirLoc<-"R:/KCA/Projects/ZeroApp_multiple/Updated_files/AGRF_trial/"
    }
    tryCatch(
      expr = {
        fileName_1 <- list.files(path = dirLoc, pattern = "^Gene[[:print:]]*TPM_Counts.txt$", full.names = T)
        tpm_online<<-read.delim(fileName_1,sep="\t",header=T, stringsAsFactors = F)
        ...
        cncInfo_online<<-read.delim(filename[1],sep="\t",header=F,row.names=1)
        cncInfo_online<<-cncInfo_online[c(1,which(rownames(cncInfo_online) %in% colnames(tpm_online))),]
        cncInfo_1_online<<-read.delim(filename[1],sep="\t",header=T)
  ...
  if(input$Select_plotfile_dot == "offline_plot"){
    ...
    if(is.null(inFile_dot) ){
      ...
      tryCatch(expr = {
        ...
        if (!is.null(genes_list_offline)) {
          updateSelectizeInput(session = session,
                               inputId = "selectgenes2_dot", 
                               choices = genes_list_offline,
                               server=T)
        }
    ...
    if(is.null(inFile_dot1)){
      ...
      tryCatch(expr = {
        ...
        if(!is.null(patient_id_offline)){
          updateSelectizeInput(session = session,
                               inputId = "selectPatient_dot", 
                               choices = patient_id_offline,
                               server=T )
...
Expression_Dotplot <- function(id, geneList, tpm, cncInfo, cncInfo_1){
  ...
  if(length(geneList)>0){
    ...
    p <- "ggplot(data=gr, aes(name, mid, colour=groups, Patient_Id=Patient.ID, TPM=TPM, Diagnosis=Diagnosis, Category = Category, size=groups, shape=groups))+ geom_hline(aes(yintercept=median(gr$TPM)))+labs(y='TPM',x='')+scale_colour_manual(values=colourGroup)+scale_size_manual(values=sizeGroup)+scale_shape_manual(values=shapeGroup)"
    ...
    output$plotarea_dot <- renderPlotly({
      ggplotly(print(eval(parse(text = p))), tooltip=c("Patient_Id","TPM", "Diagnosis", "Category"))
    })
  }
...
#feb 2022
Expression_Dotplot_CT <- function(id, geneList, tpm, cncInfo, cncInfo_1){
  ...
  #   if(grpall[i]==2){groups[i]<-forceId}else if(grpall[i]==3){groups[i]<-as.character(numPat)}else{groups[i]<-"all"}
  # }
  forceCat <- paste("a",pat_cat,sep="")
  for(i in 1:length(grpall)){
    if(grpall[i]==2){groups[i]<-forceId}else if(grpall[i]==3){groups[i]<-as.character(numPat)}else{groups[i]<-forceCat}
  }
  if(length(geneList)>0){
    gene<-geneList
    index<-which(tpm$gene_id %in% gene)
    ...
    #suppressWarnings({
      if(numCnc == 1){
        ...
        pl<-ggplot(grpdat,aes(name,TPM))+
          geom_hline(aes(yintercept=median(grpdat$TPM)), size = 0.1)+
          geom_dotplot(binaxis="y",stackdir="center",aes(fill=groups),binwidth=maxVal/60,dotsize=1.2, width = 0.8, alpha = 0.8)+
          scale_fill_manual(name="",labels=c(pat_cat,as.character(id)),values=c("blue","red"))+
          theme_classic()+theme(axis.text.x = element_text(size=25),axis.title.y=element_text(size=25))+
          labs(y="TPM",x="")
      }else{
        pl<-ggplot(grpdat,aes(name,TPM))+
          geom_hline(aes(yintercept=median(grpdat$TPM)), size = 0.1)+
          geom_dotplot(binaxis="y",stackdir="center",aes(fill=groups),binwidth=maxVal/60,dotsize=1.2, width = 0.8, alpha = 0.8)+
          scale_fill_manual(name="",labels=c(pat_cat,as.character(cncType),as.character(id)),values=c("blue","green","red"))+
          theme_classic()+theme(axis.text.x = element_text(size=25),axis.title.y=element_text(size=25))+
          labs(y="TPM",x="")
      }
      filename <- paste("Gene_", gene, "-rnaseq_personalised_",cncType,".png", sep = "")
   # })
    
    
    #Create download handler
    output$ExpressionDownload_dot_cat <- downloadHandler(
      filename = function(){filename},
      content = function(file){ggsave(filename = file, plot = pl, device = "png", width = 8)}
    )
    shinyjs::enable("ExpressionDownload_dot_cat")
    grpdat$TPM <- round(grpdat$TPM, 3)
    ...
    gr$bins <- as.character(gr$bins)
    gr$groups <- as.character(gr$groups)
    gr$colour[grep("^a", gr$groups)] <- "blue"
    gr$colour[grep("^z", gr$groups)] <- "red"
    gr$colour[!(grepl("^a",gr$groups)) & !(grepl("^z", gr$groups))] <- "green"
    #gr$groups[grep("^z", gr$groups)] <- as.character(gr$Patient.ID[grep("^z", gr$groups)])
    #gr$groups[grep("^z", gr$groups)] <- as.character(gr[grep("^z", gr$groups),5])
    gr$groups <- factor(gr$groups, 
                        levels = c(paste("a",pat_cat,sep=""), 
                                   cncType, paste("z",id,sep="")), 
                        labels = c( pat_cat, cncType,id))
    #gr$groups[gr$groups=="all"] <- "Cohort"
    colourGroup <- gr$colour
    names(colourGroup) <- gr$groups
    gr$size[gr$colour=="blue"] <- 2
    gr$size[gr$colour=="green"] <- 3
    gr$size[gr$colour=="red"] <- 4
    sizeGroup <- gr$size
    names(sizeGroup) <- gr$groups
    gr$shape[gr$colour=="blue"] <- 16
    gr$shape[gr$colour=="green"] <- 15
    gr$shape[gr$colour=="red"] <- 17
    shapeGroup <- gr$shape
    names(shapeGroup) <- gr$groups
    ...
    set.seed(1234) ## ADDED TO KEEP CONSISTENT PLOTS WHEN CHANGING PATIENT OR GENE
    output$plotarea_dot_cat <- renderPlotly({
      ggplotly(print(eval(parse(text = p))), tooltip=c("Patient_Id","TPM", "Diagnosis", "Category"))
    })
  }
  #return(pl, filename, p, tooltip)
     ...
     # expr = {
        if(nchar(input$selectgenes2_dot) > 0 & nchar(input$selectPatient_dot) > 0){
          if ((ncol(tpm_online) - 2) > nrow(cncInfo_1_online)){
            index <- which(colnames(tpm_online) %in% cncInfo_1_online[,1])
            missing_pat <- colnames(tpm_online)[-index]
            missing_pat_1 <- missing_pat[-c(1,2)]
            print(missing_pat_1)
            
            for (i in 1:length(missing_pat_1)) {
              if(i==1){
                mes2 <- paste("Warning: Patient ID's ", missing_pat_1[i], sep="")
              }else if(i==length(missing_pat_1)){
                mes2 <- paste(mes2," and ", missing_pat_1[i]," have no entries within the Patient Diagnosis file.",sep="")
              }else{
                mes2 <- paste(mes2,missing_pat_1[i],sep=",")
              }
            }
          ...
          index <- which(colnames(tpm_online) %in% cncInfo_1_online[,1])
          tpm_online <<- tpm_online[,c(1,2,index)]
          ...
          pat_cat <<- cncInfo_1_online$Category[which(cncInfo_1_online$Patient.ID == input$selectPatient_dot)]
          
         
          cncInfo_1_online_cat <<- cncInfo_1_online[which(cncInfo_1_online$Category == pat_cat),]
          
          cncInfo_online_cat <<- cncInfo_online[which(rownames(cncInfo_online) %in% cncInfo_1_online_cat$Patient.ID),]
          cncInfo_online_cat <<- rbind(cncInfo_online[1,], cncInfo_online_cat)
          
          
          index <- which(colnames(tpm_online) %in% cncInfo_1_online_cat[,1])
          tpm_online_cat <<- tpm_online[,c(1,2,index)]
          
          Expression_Dotplot(input$selectPatient_dot,input$selectgenes2_dot,tpm_online, cncInfo_online, cncInfo_1_online)
          Expression_Dotplot_CT(input$selectPatient_dot,input$selectgenes2_dot,tpm_online_cat, cncInfo_online_cat, cncInfo_1_online_cat)
        }
   # },
    #   error = function(e){
    #    return(NULL)
      ...
      expr = {
        if(nchar(input$selectgenes2_dot) > 0 & nchar(input$selectPatient_dot) > 0){
          if ((ncol(tpm_offline) - 2) > nrow(cncInfo_1_offline)){
            index <- which(colnames(tpm_offline) %in% cncInfo_1_offline[,1])
            missing_pat <- colnames(tpm_offline)[-index]
            missing_pat_1 <- missing_pat[-c(1,2)]
            print(missing_pat_1)
            # mes1 <- paste("The metadata file is missing the following patient information: " )
          ...
          tpm_offline <<- tpm_offline[,c(1,2,index)]
          
          
          #Expression_Dotplot(id, geneList, tpm, cncInfo, cncInfo_1)
          #id = input$selectPatient, geneList = input$selectgenes, tpm = gene file, cncInfo = Patient meta data, cncInfo_1 = Patient meta data w/ header = T
          
          pat_cat <<- cncInfo_1_offline$Category[which(cncInfo_1_offline$Patient.ID == input$selectPatient_dot)]
          
          
          cncInfo_1_offline_cat <<- cncInfo_1_offline[which(cncInfo_1_offline$Category == pat_cat),]
          
          cncInfo_offline_cat <<- cncInfo_offline[which(rownames(cncInfo_offline) %in% cncInfo_1_offline_cat$Patient.ID),]
          cncInfo_offline_cat <<- rbind(cncInfo_offline[1,], cncInfo_offline_cat)
          
          index <- which(colnames(tpm_offline) %in% cncInfo_1_offline_cat[,1])
          tpm_offline_cat <<- tpm_offline[,c(1,2,index)]
          
          
          ##feb 2022
          Expression_Dotplot(input$selectPatient_dot,input$selectgenes2_dot,tpm_offline, cncInfo_offline, cncInfo_1_offline)
          Expression_Dotplot_CT(input$selectPatient_dot,input$selectgenes2_dot,tpm_offline_cat, cncInfo_offline_cat, cncInfo_1_offline_cat)
          #Expression_Dotplot_CT(input$selectPatient_dot,input$selectgenes2_dot,tpm_offline, cncInfo_offline, cncInfo_1_offline)
        }
